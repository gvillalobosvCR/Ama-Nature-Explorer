...
export default function SpeciesDetailView({ pointNumber, onClose }: SpeciesDetailViewProps) {
  const { lang, offlineData, discoverPoint } = useGuide();
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-emerald-950 p-6 max-w-md mx-auto">
  ...
  if (!point) {
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-emerald-950 p-6 text-center max-w-md mx-auto">
        ...
        <button
          onClick={onClose}
          className="mt-6 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 px-6 rounded-xl text-xs transition-all shadow-lg cursor-pointer active:scale-95"
        >
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-emerald-950 text-slate-100 pb-20 max-w-md mx-auto shadow-2xl flex flex-col justify-between">
      ...
      <div className="relative h-72 w-full bg-emerald-900 overflow-hidden flex-shrink-0">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 left-4 bg-emerald-950/80 border border-emerald-800/40 text-emerald-400 hover:text-white w-10 h-10 rounded-xl flex items-center justify-center backdrop-blur-sm shadow-md transition-all active:scale-90 cursor-pointer text-xl font-bold"
        >
          ‹
        </button>
      ...
      <div className="flex-1 flex flex-col p-5 gap-5">
        ...
        {tabsList.length > 0 && (
          <div className="flex flex-col gap-3">
            ...
            {/* Tabs Content */}
            <div className="bg-emerald-900/35 border border-emerald-800/20 rounded-2xl p-4 shadow-inner min-h-24">
              {activeTab === 'habitat' && (
                <div>
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-emerald-950 border-t border-emerald-900 flex justify-around items-center h-16 px-4 z-50 shadow-2xl">
        <button
          onClick={onClose}
          className="flex flex-col items-center justify-center w-1/3 h-full text-emerald-400 font-black text-[10px] cursor-pointer"
        >
          <span className="text-lg">🌿</span>
        ...
        <button
          ...
          className="flex flex-col items-center justify-center w-1/3 h-full text-emerald-700 hover:text-emerald-500 font-black text-[10px] cursor-pointer"
        >
